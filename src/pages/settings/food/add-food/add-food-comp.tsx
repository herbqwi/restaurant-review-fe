...
const AddFood = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        await foodController.createNewFood(Food, selectedElement || "");
    };
    ...
    return <div>
        <ContentContainer title="واجهة اضافة طعام للمطعم المحدد" subtitle="قم باضافة اشهى الاطعمه" savable={true} handleSubmit={handleSubmit}>
            <div>
            ...
            <div>
                <div>
                    <div className='CAT-LAB'><label htmlFor="">اختار نوع الطعام</label>   <select id="food-Type" name="category" className="select-control2" onChange={(e) => {
                        ...
                        {Object.keys(IRestaurant.CuisineInfo).map((cuisine, index) => {
                            return <option key={index} value={cuisine}>{IRestaurant.CuisineInfo[cuisine as unknown as IRestaurant.Cuisine].name}</option>;
                        })}
                    </select></div>
